...
struct TriggerBuildsCommand: AsyncCommand {
    ...
    func run(using context: CommandContext, signature: Signature) async throws {
        ...
        do {
            try await AppMetrics.push(client: context.application.client, jobName: "trigger-builds")
        } catch {
    ...
    await withThrowingTaskGroup(of: Void.self) { group in
        for trigger in triggers {
            if let packageName = trigger.packageName, let reference = trigger.reference {
                Current.logger()?.info("Triggering \(pluralizedCount: trigger.pairs.count, singular: "build") for package name: \(packageName), ref: \(reference)")
            } else {
                Current.logger()?.info("Triggering \(pluralizedCount: trigger.pairs.count, singular: "build") for version ID: \(trigger.versionId)")
            }
