...
struct SearchFilterParser {
    ...
    func parse(term: String) -> SearchFilter? {
        ...
        guard let key = SearchFilterKey(rawValue: components[0]) else { return nil }
        let searchTerm = components[1]

        // Operator
        guard let comparator = SearchFilterPredicate(searchTerm: searchTerm) else {
            return nil
        }

        AppMetrics.apiSearchGetWithFilterTotal?.inc(1, .init(key: key))

        return try? key.searchFilter.init(value: comparator.value, comparison: comparator.operator)
    }
