...
The data in this index originates from [this master list of repositories](https://github.com/SwiftPackageIndex/MasterPackageList/blob/master/packages.json). When someone adds a repository to the master list, this site immediately makes a full clone of the package source. Metadata is extracted from the source code and local git history, as well as from the hosted repository on GitHub.
