...
Like most package managers, SwiftPM supports using git-based packages and registry-based packages. You will probably be familiar with how a git-based dependency looks in a `Package.swift` file:
...
Package registries address the main downsides of git-based dependencies. Package versions are immutable once published, a registry delivers source code in an archive without any git history, minimizing download size. SwiftPM added [support for package registries](https://github.com/swiftlang/swift-package-manager/blob/main/Documentation/PackageRegistry/PackageRegistryUsage.md) in Swift 5.7, so it’s available for use right now.
