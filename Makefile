...
test-docker:
	...
	docker run --rm -v "$(PWD)":/host -w /host \
	  ...
	  registry.gitlab.com/finestructure/spi-base:0.7.0 \
	  make test
...
build-front-end:
	docker run --rm -v $$PWD:/host -w /host --entrypoint sh node:16-alpine -c "/usr/local/bin/yarn && /usr/local/bin/yarn build"
