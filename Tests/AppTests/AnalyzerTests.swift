...
@Suite struct AnalyzerTests {
    ...
    @Test func analyze() async throws {
        try await withApp(.testing) { app, _ in
            ...
            #expect(products.map(\.name) == ["p1", "p1", "p1", "p2", "p2", "p2"])
            #expect(products.map(\.targets) == [["t1"], ["t1"], ["t1"], ["t2"], ["t2"], ["t2"]])
            #expect(products.map(\.type) == [.executable, .executable, .executable, .library(.automatic), .library(.automatic), .library(.automatic)])
...
#warning("FIXME")
    ...
    @Test func diffVersions() async throws {
        try await withApp(.testing) { app, _ in
            ...
            // validate
            #expect(delta.toAdd.map(\.reference) == [.branch("main"), .tag(1, 2, 3)])
            #expect(delta.toAdd.map(\.commit) == ["sha.main", "sha.1.2.3"])
            #expect(delta.toAdd.map(\.commitDate) == [Date(timeIntervalSince1970: 0), Date(timeIntervalSince1970: 1)])
            #expect(delta.toAdd.map(\.url) == [
    ...
    @Test func updatePackages() async throws {
        try await withApp(.testing) { app, _ in
            ...
            // MUT
            try await App.updatePackages(client: app.client,
                                         database: app.db,
                                         results: results,
                                         stage: .analysis)
            ...
            do {
                ...
                #expect(packages.map(\.status) == [.noValidVersions, .ok])
                #expect(packages.map(\.processingStage) == [.analysis, .analysis])
            }
        }
    }
    ...
    @Test func test_issue_70() async throws {
        try await withApp(.testing) { app, _ in
    ...
    @Test func issue_914() async throws {
        try await withApp(.testing) { app, _ in
            ...
            do {
                ...
                let repoDir = try Current.fileManager.checkoutsDirectory() + "/" + #require(pkg.cacheDirectoryName)
    ...
    @Test func trimCheckouts() async throws {
        try await withApp(.testing) { app, _ in
            // setup
