...
class GithubTests: AppTestCase {
    ...
    func test_funding_decodingFromYaml() throws {
        let yaml = """
        community_bridge: communityBridgeUsername
        custom: example.com/customUsername1
        github: gitHubUsername1
        issuehunt: issueHuntUsername
        ko_fi: koFiUsername
        liberapay: liberaPayUsername
        open_collective: openCollectiveUsername
        otechie: otechieUsername
        patreon: patreonUsername
        tidelift: tideLiftUsername
        """

        // MUT
        let result = try YAMLDecoder().decode(Github.Funding.self, from: yaml)

        // Validation
        XCTAssertEqual(result.communityBridge, "communityBridgeUsername")
        XCTAssertEqual(result.customUrls, ["example.com/customUsername1"])
        XCTAssertEqual(result.github, ["gitHubUsername1"])
        XCTAssertEqual(result.issueHunt, "issueHuntUsername")
        XCTAssertEqual(result.koFi, "koFiUsername")
        XCTAssertEqual(result.liberaPay, "liberaPayUsername")
        XCTAssertEqual(result.openCollective, "openCollectiveUsername")
        XCTAssertEqual(result.otechie, "otechieUsername")
        XCTAssertEqual(result.patreon, "patreonUsername")
        XCTAssertEqual(result.tideLift, "tideLiftUsername")
    }

    func test_funding_decodingFromYaml_withArrays() throws {
        let yaml = """
        custom: [example.com/customUsername1, example.com/customUsername2]
        github: [gitHubUsername1, gitHubUsername2]
        """
        ...
        // Validation
        XCTAssertEqual(result.customUrls, ["example.com/customUsername1", "example.com/customUsername2"])
        XCTAssertEqual(result.github, ["gitHubUsername1", "gitHubUsername2"])
        XCTAssertEqual(result.communityBridge, nil)
        XCTAssertEqual(result.issueHunt, nil)
        XCTAssertEqual(result.koFi, nil)
        XCTAssertEqual(result.liberaPay, nil)
        XCTAssertEqual(result.openCollective, nil)
        XCTAssertEqual(result.otechie, nil)
        XCTAssertEqual(result.patreon, nil)
        XCTAssertEqual(result.tideLift, nil)
    }
