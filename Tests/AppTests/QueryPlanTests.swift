...
class QueryPlanTests: XCTestCase {
    ...
    func test_actualTime_parse() throws {
        XCTAssertEqual(
            try QueryPlan.actualTime.parse("actual time=8.340..44.485 rows=121 loops=1"),
    ...
    func test_parser() throws {
        XCTAssertEqual(
            try QueryPlan.parser.parse("Append  (cost=412.37..3826.71 rows=81 width=308) (actual time=8.340..44.485 rows=121 loops=1)"),
    ...
    func test_init() throws {
        let input = #"""
            ...
            """#
        XCTAssertEqual(try QueryPlan(input),
